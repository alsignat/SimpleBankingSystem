...
class ChainOfResponsibilityDemo {
     ...
     */
    static RequestHandler commonRequestHandler = req -> wrapInTransactionTag.then(createDigest).then(wrapInRequestTag).handle(req);// тут лямбда !!! write a combination of existing handlers here
    ...
    interface RequestHandler {
        ...
        default RequestHandler then(RequestHandler nextHandler) {
            return req -> nextHandler.handle(this.handle(req));
            };
        }
